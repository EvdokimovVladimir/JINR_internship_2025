...
energy_alpha_pure, counts_alpha_pure, alpha_mask = extract_pure_alpha_spectrum(
    energy, counts, beta_lower, beta_upper, x0_fit, sigma_fit, REF_PEAK_WIDTH_SIGMA
)
...
plt.fill_between(
    energy, counts, where=(energy >= x0_fit - REF_PEAK_WIDTH_SIGMA * sigma_fit) & (energy <= x0_fit + REF_PEAK_WIDTH_SIGMA * sigma_fit),
...
if len(energy_alpha_pure) < 0:
    os.error("len(energy_alpha_pure) < 0")
...
mask_physical_peaks = ~(
    ...
    ((energy_alpha_pure >= x0_fit - REF_PEAK_WIDTH_SIGMA * sigma_fit) & 
        (energy_alpha_pure <= x0_fit + REF_PEAK_WIDTH_SIGMA * sigma_fit))
)
...
plt.axvspan(x0_fit - REF_PEAK_WIDTH_SIGMA*sigma_fit, x0_fit + REF_PEAK_WIDTH_SIGMA*sigma_fit, color='purple', alpha=0.15, label='Интервал референсного пика', zorder=0)
plt.axvline(x=x0_fit - REF_PEAK_WIDTH_SIGMA*sigma_fit, color='purple', linestyle=':', linewidth=1, zorder=2)
plt.axvline(x=x0_fit + REF_PEAK_WIDTH_SIGMA*sigma_fit, color='purple', linestyle=':', linewidth=1, zorder=2)
